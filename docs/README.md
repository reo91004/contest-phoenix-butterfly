...
현재 working tree의 active RTL은 **countermeasure-free original baseline**이다.
기준 commit은 Stage 1 blanking이 들어가기 직전인 `cd69410`이며, RTL/board wrapper,
model, TVLA capture script, Verilator testbench를 이 baseline code state로 복구했다.

즉 현재 active code에는 datapath zero blanking, inactive dummy PRD, share memory,
random tape, arithmetic masking이 들어가 있지 않다. 이전 논문 후보와 masking full-pass
후보는 아래 문서와 commit history에 보존한다.
...
11. `docs/experiments/arithmetic_masking/README.md`
   - ML-KEM/ML-DSA arithmetic masking 실험 기록. 현재 active RTL에는 적용하지 않는다.
     1000/1000 fixed-vs-random TVLA에서 여섯 operation이 모두 threshold 아래로 내려갔다.
...
| `docs/experiments/stage2_dsa_mul_only_prd.md` | 이전 논문 후보 RTL 최종 보고서 |
| `docs/experiments/arithmetic_masking/` | ML-KEM/ML-DSA arithmetic masking 실험 |
...
- ML-KEM/ML-DSA arithmetic masking 후보는 commit `5a81c6c`에 보존했다. 기능/Verilator/
  CW305 30ns timing은 통과했고, bitstream은 2026-05-21 13:28:47 KST에 재생성했다.
  1000/1000 TVLA에서 `mlkem_ntt=2.904`, `mlkem_intt=3.451`, `mlkem_pwm=3.147`,
  `mldsa_ntt=3.435`, `mldsa_intt=4.053`, `mldsa_pwm=2.969`로 여섯 operation이 모두
  threshold 4.5 아래였다.
- 현재 active RTL은 countermeasure-free baseline이다. 2026-05-21 16:00:40 KST에
  CW305 30ns bitstream을 재생성했고, utilization은 LUT `9329`, FF `3177`, RAMB36 `8`,
  DSP `0`, timing은 WNS `0.055 ns`, TNS `0`, WHS `0.078 ns`, THS `0`이다.
