...
# USB interface stays at 100 MHz. PHOENIX crypto clock is constrained for a
# 100 MHz timing experiment; host software must configure/provide the matching clock.
create_clock -period 10.000 -name usb_clk -waveform {0.000 5.000} [get_nets usb_clk]
create_clock -period 8.0 -name tio_clkin -waveform {0.000 4.000} [get_nets tio_clkin]
create_clock -period 8.0 -name pll_clk1 -waveform {0.000 4.000} [get_nets pll_clk1]

set_clock_groups -asynchronous \
                 -group [get_clocks usb_clk ] \
                 -group [get_clocks {tio_clkin pll_clk1}]

# both crypto input clocks have same properties so there is no point in doing
...
# Clock dedicated route override (CW305 board limitation)
set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets -quiet tio_clkin_IBUF]
set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets -quiet U_clocks/usb_clk_bufg]
